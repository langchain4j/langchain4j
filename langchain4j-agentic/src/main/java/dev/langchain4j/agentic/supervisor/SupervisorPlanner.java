    ...
    private boolean writeArgumentToScope(AgenticScope agenticScope, AgentInstance agent, String key, Object value) {
        if (agenticScope.hasState(key)) {
            ...
            if (argType != null) {
                ...
                if (argType.isAssignableFrom(existingValue.getClass())
                        && !(value.getClass().isAssignableFrom(argType))) {
                    // avoid overwriting a structured state with an unstructured argument generated from supervisor's
                    // LLM response
                    return false;
                }
            }
