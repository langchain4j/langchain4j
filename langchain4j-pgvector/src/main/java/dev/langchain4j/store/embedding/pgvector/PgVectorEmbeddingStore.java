...
public class PgVectorEmbeddingStore implements EmbeddingStore<TextSegment> {
  ...
  public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest request) {
    ...
    String whereParams = builderWhereParams(request.filter());
    try (Connection connection = setupConnection()) {
      String referenceVector = Arrays.toString(request.queryEmbedding().vector());
      String query = String.format(
              "WITH temp AS (SELECT (2 - (embedding <=> '%s')) / 2 AS score, embedding_id, embedding, text, metadata FROM %s %s) SELECT * FROM temp WHERE score >= %s ORDER BY score desc LIMIT %s;",
              referenceVector, table, whereParams, request.minScore(), request.maxResults());
  ...
  private String builderWhereParams(Filter filter) {
    if (filter == null) {
      return "";
    }
    String where = PgVectorMetadataFilterMapper.map(filter);
