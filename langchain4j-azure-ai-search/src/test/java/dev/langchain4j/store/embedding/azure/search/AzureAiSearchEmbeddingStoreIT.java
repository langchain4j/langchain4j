...
public class AzureAiSearchEmbeddingStoreIT extends EmbeddingStoreIT {
    ...
    void testAddEmbeddingsAndFindRelevant() {
        ...
        List<EmbeddingMatch<TextSegment>> relevant = embeddingStoreWithSimilarity.findRelevant(relevantEmbedding, 3);
    ...
    void testAddEmbeddingsAndFindRelevantWithHybrid() {
        ...
        for (String content : contents) {
            TextSegment textSegment = TextSegment.from(content);
            Embedding embedding = embeddingModel.embed(content).content();
            embeddingStoreWithHybrid.add(embedding, textSegment);
        }
    ...
    void testAddEmbeddingsAndFindRelevantWithSemantic() {
        ...
        for (String content : contents) {
            TextSegment textSegment = TextSegment.from(content);
            Embedding embedding = embeddingModel.embed(content).content();
            embeddingStoreWithSemantic.add(embedding, textSegment);
        }
    ...
    protected EmbeddingStore<TextSegment> embeddingStore() {
        return embeddingStoreWithSimilarity;
    }
