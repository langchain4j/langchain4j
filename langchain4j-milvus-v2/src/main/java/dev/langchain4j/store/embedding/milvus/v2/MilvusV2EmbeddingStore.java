...
import static dev.langchain4j.store.embedding.milvus.v2.CollectionOperationsExecutor.flush;
import static dev.langchain4j.store.embedding.milvus.v2.CollectionOperationsExecutor.hasCollection;
import static dev.langchain4j.store.embedding.milvus.v2.CollectionOperationsExecutor.insert;
import static dev.langchain4j.store.embedding.milvus.v2.CollectionOperationsExecutor.loadCollectionInMemory;
import static dev.langchain4j.store.embedding.milvus.v2.CollectionOperationsExecutor.removeForVector;
...
import static dev.langchain4j.store.embedding.milvus.v2.Generator.generateRandomIds;
import static dev.langchain4j.store.embedding.milvus.v2.Mapper.toEmbeddingMatches;
import static dev.langchain4j.store.embedding.milvus.v2.Mapper.toMetadataJsons;
import static dev.langchain4j.store.embedding.milvus.v2.Mapper.toScalars;
...
import static dev.langchain4j.store.embedding.milvus.v2.MilvusV2MetadataFilterMapper.map;
...
public class MilvusV2EmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest request) {
        MilvusV2EmbeddingSearchRequest milvusRequest;
        if (request instanceof MilvusV2EmbeddingSearchRequest) {
            milvusRequest = (MilvusV2EmbeddingSearchRequest) request;
        } else {
            milvusRequest = MilvusV2EmbeddingSearchRequest.milvusBuilder()
        ...
        if (Objects.equals(this.searchMode, SearchMode.HYBRID)) {
            ...
            if (embeddingSearchRequest.queryEmbedding() == null || !hasSparse) {
                throw new IllegalArgumentException(
                        "HYBRID requires dense queryEmbedding and either sparseEmbedding or query");
            }
    ...
    public void addAll(List<String> ids, List<Embedding> embeddings, List<TextSegment> textSegments) {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            // Only HYBRID CUSTOM collections have a sparse field that needs to be explicitly set
            if (this.searchMode == SearchMode.HYBRID && this.sparseMode == MilvusSparseMode.CUSTOM) {
                row.add(fieldDefinition.getSparseVectorFieldName(), gson.toJsonTree(new TreeMap<>()));
            }
    ...
    public static class Builder {
         ...
         *                    BM25 - use Milvus built-in sparse vector generation from text (query in search request must be provided).
        ...
        /**
         * @param searchMode  The search mode for this store.
         *                    VECTOR - dense vector search only (default).
         *                    HYBRID - hybrid dense + sparse search.
         *                    Default value: VECTOR.
         * @return builder
