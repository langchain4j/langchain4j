...
public class MistralAiChatModel implements ChatModel {
    ...
    private ChatRequestParameters initDefaultRequestParameters(String modelName,
        ...
        return DefaultChatRequestParameters.builder()
                .modelName(ensureNotBlank(getOrDefault(modelName, commonParameters.modelName()), "modelName"))
                .temperature(getOrDefault(temperature, commonParameters.temperature()))
                .topP(getOrDefault(topP, commonParameters.topP()))
                .maxOutputTokens(getOrDefault(maxTokens, getOrDefault(commonParameters.maxOutputTokens(), 1024)))
    ...
    public ChatResponse doChat(ChatRequest chatRequest) {
        ...
        Response<AiMessage> response = Response.from(
                aiMessageFrom(mistralAiResponse),
                tokenUsageFrom(mistralAiResponse.getUsage()),
                finishReasonFrom(mistralAiResponse.getChoices().get(0).getFinishReason()));
