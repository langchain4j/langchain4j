 ...
 * When using a service account, ensure that <code>GOOGLE_APPLICATION_CREDENTIALS</code> environment variable points to your JSON service account key.
 * <br>
...
public class VertexAiGeminiChatModel implements ChatModel, Closeable {
    ...
    private static final ExceptionMapper EXCEPTION_MAPPER = ExceptionMapper.DEFAULT;
    ...
    public VertexAiGeminiChatModel(
        ...
        if (safetySettings != null) {
            this.safetySettings = new EnumMap<>(safetySettings);
        } else {
        ...
        this.listeners = listeners == null ? emptyList() : List.copyOf(listeners);
    }

    public VertexAiGeminiChatModel(GenerativeModel generativeModel, GenerationConfig generationConfig) {
        this(generativeModel, generationConfig, DEFAULT_MAX_RETRIES);
    }
    ...
    private Response<AiMessage> generate(List<ChatMessage> messages, List<ToolSpecification> toolSpecifications) {
        ...
        with(instructionAndContent.systemInstruction, model::withSystemInstruction);
        ...
        try {
            response = withRetryMappingExceptions(
                    () -> finalModel.generateContent(instructionAndContent.contents), maxRetries, EXCEPTION_MAPPER);
        } catch (Exception e) {
            listeners.forEach(listener -> {
                try {
            ...
            if (e instanceof DeadlineExceededException deadlineExceededException) {
                logger.warn("Timeout while calling model", e);
                throw new TimeoutException(deadlineExceededException);
            } else if (e instanceof ApiException apiException) {
                final var statusCode = ((HttpJsonStatusCode) apiException.getStatusCode()).getTransportCode();
                logger.warn("Error while calling model (status code: {})", statusCode, e);
                throw EXCEPTION_MAPPER.mapException(new HttpException(statusCode, e));
            } else {
                logger.warn("Error while calling model", e);
                throw EXCEPTION_MAPPER.mapException(e);
            }
        }
        ...
        final ChatResponse listenerResponse = ChatResponse.builder()
                .aiMessage(aiMessage)
                .metadata(ChatResponseMetadata.builder()
                        .modelName(modelName)
    ...
    public static class VertexAiGeminiChatModelBuilder {
        ...
        public VertexAiGeminiChatModelBuilder vertexAI(VertexAI vertexAI) {
