...
public final class GoogleAiGeminiBatchChatModel extends BaseGeminiChatModel {
     ...
     */
    public sealed interface BatchResponse permits BatchIncomplete, BatchChatSuccess, BatchEmbedSuccess, BatchError {
    }
     ...
     */
    public record BatchChatSuccess(BatchName batchName, List<ChatResponse> responses) implements BatchResponse {
    }

    public record BatchEmbedSuccess(BatchName batchName,
                                    List<GeminiEmbeddingResponse> responses) implements BatchResponse {
    }
    ...
    public BatchResponse createEmbedBatchInline(String displayName, @Nullable Long priority, List<TextSegment> requests) {
//        var modelName = extractModelFromEmbedRequests(requests);
        var inlineRequests = requests.stream()
                // Merge the chat requests with the values set in the Builder.
                .map(this::applyDefaultParameters)
                // Create a Gemini specific content requests for each ChatRequest.
     ...
     *   <li>{@link BatchChatSuccess} - if the (chat) batch completed successfully, containing all responses</li>
     *   <li>{@link BatchEmbedSuccess} - if the (embed) batch completed successfully, containing all responses</li>
    ...
    private static String extractModelFromEmbedRequests(List<GeminiEmbeddingRequest> requests) {
        var modelNames = requests.stream().map(GeminiEmbeddingRequest::model).collect(Collectors.toUnmodifiableSet());
        if (modelNames.size() != 1) {
            throw new IllegalArgumentException("Batch requests cannot contain ChatRequest objects with different "
                    + "models; all requests must use the same model.");
        }
