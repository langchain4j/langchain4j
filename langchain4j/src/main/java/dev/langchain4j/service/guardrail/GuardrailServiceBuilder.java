...
final class GuardrailServiceBuilder implements Builder {
     ...
     */
    @Override
    public Builder inputGuardrailClasses(@Nullable List<Class<? extends InputGuardrail>> guardrailClasses) {
     ...
     */
    @Override
    public Builder outputGuardrailClasses(@Nullable List<Class<? extends OutputGuardrail>> guardrailClasses) {
    ...
    public DefaultGuardrailService build() {
        ...
        ClassMetadataProvider.getClassMetadataProviderFactory()
                .getNonStaticMethodsOnClass(this.aiServiceClass)
                .forEach(method -> {
                    var inputGuardrailsForMethod = computeInputGuardrailsForAiServiceMethod(method);
                    var outputGuardrailsForMethod = computeOutputGuardrailsForAiServiceMethod(method);
    ...
    private InputGuardrailExecutor computeInputGuardrailsForAiServiceMethod(Object method) {
        // For both input & output guardrails, first check the method
        // If nothing on the method, then fall back to the class
        return ClassMetadataProvider.getClassMetadataProviderFactory()
                .getAnnotation(method, InputGuardrails.class)
                .map(GuardrailServiceBuilder::computeInputGuardrails)
                .orElseGet(this::createClassLevelInputGuardrailExecutor);
    }

    private OutputGuardrailExecutor computeOutputGuardrailsForAiServiceMethod(Object method) {
        // For both input & output guardrails, first check the method
        // If nothing on the method, then fall back to the class
        return ClassMetadataProvider.getClassMetadataProviderFactory()
                .getAnnotation(method, OutputGuardrails.class)
                .map(GuardrailServiceBuilder::computeOutputGuardrails)
                .orElseGet(this::createClassLevelOutputGuardrailExecutor);
    }

    private InputGuardrailExecutor createClassLevelInputGuardrailExecutor() {
        // At the class level, if guardrails and config are set both via the builder and annotations, then
        // the annotations win
        return ClassMetadataProvider.getClassMetadataProviderFactory()
                .getAnnotation(this.aiServiceClass, InputGuardrails.class)
                .map(GuardrailServiceBuilder::computeInputGuardrails)
    ...
    private OutputGuardrailExecutor createClassLevelOutputGuardrailExecutor() {
        // At the class level, if guardrails and config are set both via the builder and annotations, then
        // the annotations win
        return ClassMetadataProvider.getClassMetadataProviderFactory()
                .getAnnotation(this.aiServiceClass, OutputGuardrails.class)
                .map(annotation -> OutputGuardrailExecutor.builder()
                        .config(computeConfig(annotation.config()))
                        .guardrails(getGuardrails(annotation))
                        .build())
                .orElseGet(() -> OutputGuardrailExecutor.builder()
