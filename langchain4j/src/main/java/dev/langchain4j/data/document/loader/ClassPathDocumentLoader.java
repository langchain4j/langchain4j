...
public class ClassPathDocumentLoader {
	...
	public static List<Document> loadDocuments(String directoryOnClasspath, PathMatcher pathMatcher, DocumentParser documentParser) {
		return loadDocuments(directoryOnClasspath, pathMatcher, documentParser, ClassPathDocumentLoader::getFilesInDirectory);
	}

	private static List<Document> loadDocuments(String directoryOnClasspath, PathMatcher pathMatcher, DocumentParser documentParser, Function<Path, Stream<Path>> pathStreamFunction) {
		...
		try {
			...
			if (classPathSource.isInsideArchive()) {
				try (var fs = FileSystems.newFileSystem(uri, Map.of("create", "true"))) {
					return loadDocuments(classPathSource, fs.getPath(directoryOnClasspath), pathMatcher, documentParser, pathStreamFunction);
				}
			}
			else {
	...
	private static List<Document> loadDocuments(ClassPathSource rootDirectoryClassPathSource, Path path, PathMatcher pathMatcher, DocumentParser documentParser, Function<Path, Stream<Path>> pathStreamFunction) {
		if (!isDirectory(path)) {
	...
	private static List<Document> loadDocuments(Stream<Path> pathStream, ClassPathSource rootDirectoryClassPathSource, Path pathMatcherRoot, PathMatcher pathMatcher, DocumentParser documentParser) {
		return pathStream
			...
			.map(p -> {
				try {
					...
					return loadDocument(ClassPathSource.from(relativePath, rootDirectoryClassPathSource.classLoader()), p, documentParser);
				}
	...
	private static String getRelativePath(ClassPathSource rootDirectoryClassPathSource, Path subPath) {
		...
		try {
			var rootClasspathURI = rootDirectoryClassPathSource.classLoader()
			                                                   .getResource(".")
			                                                   .toURI();
	...
	public static List<Document> loadDocumentsRecursively(String directoryOnClasspath, PathMatcher pathMatcher, DocumentParser documentParser) {
		return loadDocuments(directoryOnClasspath, pathMatcher, documentParser, ClassPathDocumentLoader::getFilesInDirectoryRecursively);
	}
