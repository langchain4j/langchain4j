...
class ClassPathDocumentLoaderTest implements WithAssertions {
	...
	void should_load_text_document(String path, boolean isInsideArchive) {
		...
		assertThat(document.metadata().getString(Document.URL))
			.isEqualTo(Thread.currentThread().getContextClassLoader().getResource(path).getFile());
	...
	static Stream<Arguments> should_recursively_load_documents_arguments() {
		return Stream.of(
			Arguments.of(CLASSPATH_CHECK_DIRECTORY, List.of("test-file-3.banana", "test-file-4.banana", "file1.txt", "file2.txt")),
			Arguments.of(CLASSPATH_IN_ARCHIVE_CHECK_DIRECTORY, List.of("test-file-5.banana", "test-file-6.banana", "file3.txt", "file4.txt"))
		);
	}
	...
	void should_recursively_load_matching_documents_with_glob_crossing_directory_boundaries(String path, List<String> expectedFileNames) {
	...
	static Stream<Arguments> should_recursively_load_matching_documents_with_glob_crossing_directory_boundaries_arguments() {
		return Stream.of(
			...
			Arguments.of(CLASSPATH_IN_ARCHIVE_CHECK_DIRECTORY, List.of("test-file-5.banana", "test-file-6.banana"))
		);
	}
	...
	@CsvSource(delimiter = '|', textBlock = """
		...
		""")
	void should_recursively_load_matching_documents_with_glob_specifying_concrete_directory(String path, String expectedFileName) {
