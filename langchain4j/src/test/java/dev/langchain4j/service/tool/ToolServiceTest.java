...
class ToolServiceTest {
    ...
    @Test
    void findTools_fails_on_autogenerated_parameter_name_when_strict() {
        class Tools {
            @Tool
            public void tool(String foo) {}
        }

        assertThatThrownBy(() -> ToolService.findTools(new Tools(), true))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("-parameters");
    }

    @Test
    void findTools_does_not_fail_on_autogenerated_parameter_name_by_default() {
        class Tools {
            @Tool
            public void tool(String foo) {}
        }

        assertThatNoException().isThrownBy(() -> ToolService.findTools(new Tools()));
    }

    @Test
    void findTools_does_not_fail_on_parameter_with_explicit_P_name_when_strict() {
        class Tools {
            @Tool
            public void tool(@P(name = "foo") String foo) {}
        }

        assertThatNoException().isThrownBy(() -> ToolService.findTools(new Tools(), true));
    }
}
