...
class AiServicesWithRagIT {
    ...
    void should_use_LLM_generated_metadata_filter(ChatLanguageModel model) {
        ...
        // given
        TextSegment groundhogDay = TextSegment.from(
                "Groundhog Day",
                new dev.langchain4j.data.document.Metadata()
                        .put("genre", "comedy")
                        .put("year", 1993));
        TextSegment forrestGump =
                TextSegment.from("Forrest Gump", metadata("genre", "drama").put("year", 1994));
        TextSegment dieHard =
                TextSegment.from("Die Hard", metadata("genre", "action").put("year", 1998));
    ...
    void should_use_legacy_retriever(ChatLanguageModel model) {
        ...
        // given
        Retriever<TextSegment> legacyRetriever = EmbeddingStoreRetriever.from(embeddingStore, embeddingModel, 1);
