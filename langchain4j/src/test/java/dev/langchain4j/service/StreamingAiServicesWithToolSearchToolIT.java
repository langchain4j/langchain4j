...
public class StreamingAiServicesWithToolSearchToolIT {
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__keep_previously_found_tools_in_chat_memory(StreamingChatModel model)
            throws Exception {

        // given
        StreamingChatModel spyModel = spy(model);
        AiServicesWithToolSearchToolIT.SearchableTools spyTools =
                spy(new AiServicesWithToolSearchToolIT.SearchableTools());
        ToolSearchStrategy spyToolSearchStrategy = spy(new SimpleToolSearchStrategy());
        Consumer<BeforeToolExecution> spyBeforeToolExecution =
                spy(new AiServicesWithToolSearchToolIT.BeforeToolExecutionCallback());
        Consumer<ToolExecution> spyAfterToolExecution =
                spy(new AiServicesWithToolSearchToolIT.AfterToolExecutionCallback());
        ...
        InOrder inOrder = inOrder(
                spyModel, spyToolSearchStrategy, spyTools, spyBeforeToolExecution, spyAfterToolExecution, handler);

        inOrder.verify(spyModel)
                .chat(
                        argThat((ChatRequest request) ->
                                request.toolSpecifications().size() == 1 && containsTool(request, "tool_search_tool")),
                        any());

        inOrder.verify(spyBeforeToolExecution)
                .accept(argThat(bte -> bte.request().name().equals("tool_search_tool")
                        && bte.request().arguments().contains("weather")));
        inOrder.verify(handler)
                .beforeToolExecution(argThat(bte -> bte.request().name().equals("tool_search_tool")
                        && bte.request().arguments().contains("weather")));

        inOrder.verify(spyToolSearchStrategy)
                .search(argThat(request -> hasToolSearch(request, "tool_search_tool", "weather")
                        && hasSearchableTools(request, "getWeather", "getTime")));

        inOrder.verify(spyAfterToolExecution)
                .accept(argThat(te -> te.request().name().equals("tool_search_tool")
                        && te.result().equals("Tools found: getWeather")));
        inOrder.verify(handler)
                .onToolExecuted(argThat(te -> te.request().name().equals("tool_search_tool")
                        && te.result().equals("Tools found: getWeather")));
        ...
        inOrder.verify(spyToolSearchStrategy)
                .search(argThat(
                        request -> hasToolSearch(request, "tool_search_tool", "time")
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__keep_previously_found_tools_in_chat_memory__tool_provider(
        ...
        inOrder.verify(spyToolSearchStrategy)
                .search(argThat(request -> hasToolSearch(request, "tool_search_tool", "time")));
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__multiple_simultaneous_tool_searches(StreamingChatModel model)
            throws Exception {
        ...
        inOrder.verify(spyModel)
                .chat(
                        argThat((ChatRequest request) -> request.toolSpecifications()
                                                .size()
                                        == 3
                                && containsTool(request, "tool_search_tool")
                                && containsTool(request, "getWeather")
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__multiple_simultaneous_tool_searches__tool_provider(StreamingChatModel model)
            throws Exception {
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__multiple_simultaneous_tool_searches__overlapping_tools(
            StreamingChatModel model) throws Exception {
        ...
        ToolSearchStrategy spyToolSearchStrategy = spy(new ToolSearchStrategy() {
            ...
            public ToolSearchResult search(ToolSearchRequest request) {
                ...
                List<String> foundToolNames = request.searchableTools().stream()
                ...
                .chat(
                        argThat((ChatRequest request) -> request.toolSpecifications()
                                ...
                                && toolResultMessage
                                        .attributes()
                                        .get("found_tools")
                                        .equals(List.of("getTime", "getWeather"))
                                && request.messages().get(4) instanceof ToolExecutionResultMessage toolResultMessage2
                                && toolResultMessage2.toolName().equals("tool_search_tool")
        ...
        inOrder.verify(spyToolSearchStrategy)
                .search(argThat(request -> hasToolSearch(request, "tool_search_tool", "getDate")));
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__simultaneous_tool_search_and_normal_tool_call(StreamingChatModel model)
            throws Exception {
        ...
        // when
        ChatResponse chatResponse2 = chat(
                assistant,
                "What is the time in London? What is the weather in Paris? "
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool__simultaneous_tool_search_and_normal_tool_call__tool_provider(
            StreamingChatModel model) throws Exception {
    ...
    void should_support_tool_search_tool_with_always_visible_tools(StreamingChatModel model) throws Exception {
        ...
        // given
        StreamingChatModel spyModel = spy(model);
        AiServicesWithToolSearchToolIT.AlwaysVisibleTools spyAlwaysVisibleTools =
                spy(new AiServicesWithToolSearchToolIT.AlwaysVisibleTools());
        AiServicesWithToolSearchToolIT.SearchableTools spySearchableTools =
                spy(new AiServicesWithToolSearchToolIT.SearchableTools());
        ToolSearchStrategy spyToolSearchStrategy = spy(new SimpleToolSearchStrategy());
        ...
        inOrder.verify(spyToolSearchStrategy)
                .search(argThat(request ->
                        hasToolSearch(request, "tool_search_tool", "time") && hasSearchableTools(request, "getTime")));
    ...
    @ParameterizedTest
    @MethodSource("models")
    void should_support_tool_search_tool_with_always_visible_tools__tool_provider(StreamingChatModel model)
            throws Exception {
                ...
                return super.provideTools(request).toBuilder()
                        .add(addTool, new ToolExecutor() {
                            ...
                            public ToolExecutionResult execute(
                                ...
                                return ToolExecutionResult.from(String.valueOf(args.a + args.b + 17));
                            }
                        })
                        .build();
            }
    ...
    private static ChatResponse chat(
            AssistantWithToolSearch assistant, String userMessage, TestTokenStreamHandler handler) throws Exception {
        return chat(assistant, userMessage, "", handler);
    }

    private static ChatResponse chat(AssistantWithToolSearch assistant, String userMessage, String instructions)
            throws Exception {
        return chat(assistant, userMessage, instructions, new TestTokenStreamHandler());
    }

    private static ChatResponse chat(
            AssistantWithToolSearch assistant, String userMessage, String instructions, TestTokenStreamHandler handler)
            throws Exception {
