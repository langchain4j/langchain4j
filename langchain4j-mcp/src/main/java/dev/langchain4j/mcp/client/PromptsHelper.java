...
class PromptsHelper {
    ...
    static List<PromptRef> parsePromptRefs(JsonNode mcpMessage) {
        ...
        if (mcpMessage.has("result")) {
            ...
            if (resultNode.has("prompts")) {
                List<PromptRef> promptRefs = new ArrayList<>();
                for (JsonNode promptNode : resultNode.get("prompts")) {
                    promptRefs.add(OBJECT_MAPPER.convertValue(promptNode, PromptRef.class));
                }
                return promptRefs;
            } else {
    ...
    static PromptResponse parsePromptContents(JsonNode mcpMessage) {
        McpErrorHelper.checkForErrors(mcpMessage);
        return OBJECT_MAPPER.convertValue(mcpMessage.get("result"), PromptResponse.class);
    }
}
