...
class McpJsonConversionsTest {
    ...
    void an_integer_too_wide_for_a_long_should_not_be_truncated() {
        assertThat(McpJsonConversions.toMap(McpRawJson.parse(JSON)))
    ...
    void integral_values_should_keep_their_narrowest_exact_type() {
        var map = McpJsonConversions.toMap(McpRawJson.parse(JSON));
    ...
    void it_should_agree_with_the_mapper_based_conversion() {
        assertThat(McpJsonConversions.toMap(McpRawJson.parse(JSON))).isEqualTo(McpRawJson.toMap(JSON));
    }
