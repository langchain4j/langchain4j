...
public class WatsonxScoringModelTest {
    ...
    void testScoreAllWithParameters() {
        ...
        scoringModel = WatsonxScoringModel.builder().service(mockRerankService).build();

        ((WatsonxScoringModel) scoringModel).setParameters(parameters);

        result = scoringModel.scoreAll(List.of(TextSegment.from("test1"), TextSegment.from("test2")), "query");
        assertEquals(2, result.content().size());
        assertEquals(0.0, result.content().get(0));
        assertEquals(0.1, result.content().get(1));
