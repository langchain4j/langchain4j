...
import dev.langchain4j.model.wenxin.client.chat.Parameters;
import dev.langchain4j.model.output.FinishReason;
import dev.langchain4j.model.output.TokenUsage;
import dev.langchain4j.model.wenxin.client.chat.ChatCompletionResponse;
import dev.langchain4j.model.wenxin.client.chat.Message;
import dev.langchain4j.model.wenxin.client.chat.Role;
import dev.langchain4j.model.wenxin.client.chat.FunctionCall;
import dev.langchain4j.model.wenxin.client.chat.Function;
...
public class InternalWenxinHelper {
    ...
    static FinishReason finishReasonFrom(String finishReason) {
        ...
        if(StrUtil.isEmpty(finishReason)){
            return null;
        }
    ...
    public static List<Message> toOpenAiMessages(List<ChatMessage> messages) {
        return messages.stream()
                .map(InternalWenxinHelper::toErnieBotMessage)
                .collect(toList());
    }
