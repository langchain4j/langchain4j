...
class ColumnsMetadataHandler implements MetadataHandler {
    ...
    final List<String> escapedColumnsName;

    final MariaDbFilterMapper filterMapper;
    ...
    public ColumnsMetadataHandler(MetadataStorageConfig config, List<String> sqlKeywords) {
        List<String> columnsDefinitionList =
                ensureNotEmpty(config.columnDefinitions(), "Metadata definition");
        this.filterMapper = new ColumnFilterMapper();

        // enquote identifier if keywords
        this.indexes =
                ((List<String>) getOrDefault(config.indexes(), Collections.emptyList()))
                        .stream()
                                .map(
                                        key -> {
                                            if (sqlKeywords.contains(
                                                    key.toLowerCase(Locale.ROOT))) {
                                                try {
                                                    return org.mariadb.jdbc.Driver
                                                            .enquoteIdentifier(key, true);
        ...
        this.columnsDefinition =
                columnsDefinitionList.stream()
                        .map(str -> MetadataColumDefinition.from(str, sqlKeywords))
                        .collect(Collectors.toList());
        this.escapedColumnsName =
                columnsDefinition.stream()
                        .map(MetadataColumDefinition::getEscapedName)
                        .collect(Collectors.toList());
        this.columnsName =
                columnsDefinition.stream()
                        .map(MetadataColumDefinition::getName)
                        .collect(Collectors.toList());
    }
    ...
    public void createMetadataIndexes(Statement statement, String table) {
        ...
        this.indexes.stream()
                ...
                .forEach(
                        index -> {
                            sb.append(",")
                                    .append(
                                            MariaDbValidator.validateAndEnquoteIdentifier(
                                                    index, false));
                        });
        ...
        String indexSql =
                String.format(
                        ...
                        (table + "_metadata_idx").replaceAll("[ \\`\"'\\\\\\P{Print}]", ""),
                        table,
                        indexFields);
        try {
            ...
            throw new RuntimeException(
                    String.format("Cannot create indexes on %s: %s", indexFields, e));
        }
    }
