...
final class OpenAiDalleClient {
    ...
    @Builder
    public OpenAiDalleClient(@NonNull String openAiApiKey,
    ...
    private static RuntimeException toException(retrofit2.Response<?> response) throws IOException {
        return new RuntimeException(String.format("status code: %s; body: %s", response.code(), response.errorBody().string()));
    }
