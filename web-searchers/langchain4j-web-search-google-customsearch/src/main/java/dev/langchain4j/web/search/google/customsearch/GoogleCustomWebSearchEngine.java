...
import static dev.langchain4j.web.search.WebSearchInformationResult.informationResult;
import static dev.langchain4j.web.search.WebSearchOrganicResult.webSearchOrganicResult;
import static dev.langchain4j.web.search.WebSearchPagination.pagination;
import static dev.langchain4j.web.search.WebSearchResults.webSearchResults;
    ...
    public WebSearchResults search(WebSearchRequest webSearchRequest) {
        ...
        return webSearchResults(
                ...
                , search.getItems().stream()
                        .map(result -> webSearchOrganicResult(
                                    result.getTitle(),
                                    result.getLink(),
                                    result.getSnippet(),
                ...
                , pagination(
                        !isNullOrEmpty(search.getQueries().getRequest())
                                ?calculatePageNumberFromQueries(search.getQueries().getRequest().get(0)):1
                        , String.valueOf(!isNullOrEmpty(search.getQueries().getNextPage())
                                ?calculatePageNumberFromQueries(search.getQueries().getNextPage().get(0)):"") // next page value can be a link or a number. GoogleSearchAPI returns the page number.
                        , String.valueOf(!isNullOrEmpty(search.getQueries().getPreviousPage())
                                ?calculatePageNumberFromQueries(search.getQueries().getPreviousPage().get(0)):"") // previous page value can be a link or a number. GoogleSearchAPI returns the page number.
                        , null));
    }
