...
CUDA, OpenCL or Metal. It is the successor of GPULlama3.java (`langchain4j-gpu-llama3`).
...
The unit tests and the inherited `AbstractChatModelIT` / `AbstractStreamingChatModelIT` suites run
in one TornadoVM JVM, driven by the `run-tests` profile:

```bash
export JAVA_HOME=/path/to/jdk-25            # or a JDK 21, with a jdk21 SDK
...
Without `-Dit.selection` only the `*IT` classes run. Several inherited tests assert model
behaviour as well as the adapter, so results depend on the model:

| Model | Result |
| --- | --- |
| `Qwen3-0.6B-F16` | 42/42 |
| `Llama-3.2-3B-Instruct-Q8_0` | 36/42: the six tool tests call the tool correctly, but the model's final answer does not contain what the tests expect |

Tool transport itself is covered deterministically by `JitLLMConversionsTest`.
