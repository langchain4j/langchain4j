...
class BedrockChatModelTest {
    ...
    void should_fail_instantiation_no_factory_found() {
        final RuntimeException runtimeException = assertThrows(RuntimeException.class, () -> BedrockChatModel.builder()
                ...
                .build());
        assertThat(runtimeException.getMessage()).isEqualTo("No BedrockRuntimeClientFactory implementation found");
    }

    // should fail because there is no HTTP client implementation in the classpath
    @Test
    void should_fail_instantiate_bedrockruntimeclient_no_http_impl_found() {
        final RuntimeException runtimeException = assertThrows(
                RuntimeException.class,
                () -> BedrockRuntimeClient.builder().region(Region.EU_CENTRAL_1).build());
