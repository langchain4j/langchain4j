...
public class BedrockKnowledgeBaseContentRetriever implements ContentRetriever {
    ...
    private static final Logger log = LoggerFactory.getLogger(BedrockKnowledgeBaseContentRetriever.class);
    ...
    public List<Content> retrieve(Query query) {
        log.debug("Retrieving content from Knowledge Base '{}' for query: {}", knowledgeBaseId, query.text());
        ...
        for (KnowledgeBaseRetrievalResult result : response.retrievalResults()) {
            ...
            if (score != null && score < minScore) {
                log.debug("Skipping result with score {} (below minScore {})", score, minScore);
                continue;
            }
        ...
        log.debug("Retrieved {} content(s) from Knowledge Base '{}'", contents.size(), knowledgeBaseId);
