 ...
 * <p>
 * <b>Cancellation.</b> Bedrock streams over the AWS SDK's {@code converseStream} event publisher; the only abort
 * mechanism is the {@link StreamingHandle} (backed by the SDK subscription) that the streaming dispatch threads
 * through the per-callback contexts. This bridge captures that handle (see {@link #cancelUpstream()}) so that
 * cancelling the downstream subscription cancels the SDK subscription instead of letting the stream drain in the
 * background. This is a copy of the OpenAI/Anthropic modules' bridges, pending consolidation into a shared module.
 */
final class TubeBackedStreamingChatResponseHandler implements StreamingChatResponseHandler {

    private final Tube<StreamingEvent> tube;

    // The upstream StreamingHandle only becomes available once the SDK has produced its first event, so it is
    // captured lazily from the first per-callback context (see capture(...)).
    private final AtomicReference<StreamingHandle> upstreamHandle = new AtomicReference<>();

    TubeBackedStreamingChatResponseHandler(Tube<StreamingEvent> tube) {
        this.tube = ensureNotNull(tube, "tube");
    }

    /**
     * Aborts the upstream stream, if it is already running. Wire this to {@code tube.whenTerminates(...)} so a
     * downstream {@code Flow.Subscription} cancellation (or a buffer overflow) cancels the SDK subscription rather
     * than draining it. {@link StreamingHandle#cancel()} is idempotent and a no-op once the stream has ended, so
     * this is safe to call on any terminal signal.
     */
    void cancelUpstream() {
