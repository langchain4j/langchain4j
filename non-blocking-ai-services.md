...
|---|---|---|---|
| HTTP | `HttpClient` (`langchain4j-http-client`) | `execute(HttpRequest)` | `executeAsync(HttpRequest) : CompletableFuture<SuccessfulHttpResponse>`, `stream(HttpRequest[, ServerSentEventParser]) : Flow.Publisher<HttpStreamingEvent>` (JDK / OkHttp / Apache clients) |
| Model — single response | `ChatModel` | `chat`/`doChat(ChatRequest)` | `chatAsync`/`doChatAsync(ChatRequest) : CompletableFuture<ChatResponse>` |
| Model — streaming | `StreamingChatModel` | `chat`/`doChat(ChatRequest, handler)` | `chat`/`doChat(ChatRequest) : Flow.Publisher<ChatModelStreamingEvent>` (cold) |
...
- **HTTP layer** — `HttpStreamingEvent`: a parsed transport event (an SSE `ServerSentEvent`, or stream
  completion). The model client turns these into model events.
- **Model layer** — low-level `ChatModelStreamingEvent`: `PartialResponse`, `PartialThinking`, `PartialToolCall`,
...
| Concern | API |
|---|---|
| Reactive model stream | `StreamingChatModel.chat(ChatRequest) : Flow.Publisher<ChatModelStreamingEvent>` |
...
| **Reactive support for non-OpenAI model providers** | Validated across transports: async `doChatAsync` is implemented by **OpenAI** and **Anthropic** (both over the `HttpClient` abstraction) and **Bedrock** (over the AWS SDK's native async client); the reactive `doChat` publisher is implemented by **OpenAI**, **Anthropic**, and **Bedrock**, all with real mid-stream cancellation (Anthropic aborts via the SSE parser's `StreamingHandle`; Bedrock cancels the AWS SDK event-stream subscription). Remaining chat/embedding providers fall back to the fail-loud defaults and need a per-provider implementation, which the shared `langchain4j-reactive-streaming` module now makes a small change: `HttpStreamingChatPublisher` (the tube + subscription-based cancellation glue for any `HttpClient.stream()`-based provider) and the `TubeBackedStreamingChatResponseHandler` handler→publisher bridge are both extracted there and reused by OpenAI, Anthropic, and Bedrock — a new provider supplies only its per-event `Sink` (SSE payload parsing). |
...
| **Anthropic streaming fully reactive** | **Delivered.** Anthropic's streaming (both handler and reactive-publisher paths) now drives the SSE listener from `HttpClient.stream()` → the JDK client's `BodyHandlers.ofPublisher()` (nothing parked on socket reads), matching OpenAI; the same event-interpretation logic is reused, only the transport driver changed. The reactive non-blocking guarantee is enforced per provider by BlockHound TCKs: OpenAI and Anthropic share `AbstractChatModelNonBlockingIT` (in `langchain4j-reactive-streaming`), which drives both the `chatAsync` and streaming-publisher paths against a local WireMock plain-HTTP endpoint (no TLS, so the JDK's one-time truststore reads aren't false positives) and polices the JDK `HttpClient-*` workers; Bedrock has its own standalone `BedrockNonBlockingIT` (single BlockHound install) policing **both** the AWS SDK's `sdk-async-response-*` executor (the `chatAsync` parse) and its `aws-java-sdk-NettyEventLoop-*` threads (where the streaming pipeline runs). |
...
| Public-API naming `TODO`s | **Resolved** for the event layers: the three streaming-event vocabularies parallel each other — `HttpStreamingEvent` (transport) / `ChatModelStreamingEvent` (model, renamed from `StreamingEvent`) / `AiServiceStreamingEvent` (AI Service). Model-layer members stay suffix-less (they double as handler-API payloads: `onPartialResponse(PartialResponse)`); AI-Service-layer members keep the `…Event` suffix (pure envelopes) and remain nested classes (discoverable single entry point, namespaced against the observability `ToolCompensatedEvent`, evolvable by added accessors rather than record components). `CompleteResponse` kept (Partial/Complete symmetry with `CompleteToolCall`). |
| `@since` tags | **Reconciled** — branch-added async members are `@since 1.19.0`; members genuinely released in `1.17.0`/`1.13.0` are left as-is. |
