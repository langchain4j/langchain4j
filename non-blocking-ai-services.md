...
| Item | What's left |
|---|---|
| **Reactive/async for the remaining model providers** | OpenAI, Anthropic and Bedrock are done (async `doChatAsync` + reactive `doChat` publisher, with real mid-stream cancellation). Other chat/embedding providers still fall back to the fail-loud defaults. Adding one is now a small change via the shared `langchain4j-reactive-streaming` module — `HttpStreamingChatPublisher` (tube + subscription-based cancellation for any `HttpClient.stream()`-based provider) and the `TubeBackedStreamingChatResponseHandler` handler→publisher bridge are already extracted; a new provider supplies only its per-event SSE `Sink`. |
| **Per-provider async stores** | Persistent `ChatMemoryStore` / `EmbeddingStore` integrations (Redis, JDBC, …) still need their `*Async` methods implemented to be non-blocking on the async/reactive paths (they return the fail-loud default today). Bundled in-memory stores are done. |
| **`LangChain4jManaged` thread-local across async hops** | `LangChain4jManaged.CURRENT` is an LC4j-owned `ThreadLocal` that does **not** follow the new thread hops. Audit whether it is read downstream of an async offload; if so, fold it into `InvocationContext` or capture/restore it around LC4j's async boundaries. (Host context propagation is already covered by the `ExecutorProvider` seam, §3.1.1.) |
| **Kotlin `chatAsync` source compatibility** | Open decision (see **Breaking changes**): for a bare single-argument call the Kotlin `chatAsync` extension's effective return type shifts to `CompletableFuture<ChatResponse>`. Needs the Kotlin-extension owner's call before release. |
| **Relocate this design doc** | It still lives in the repo root; move under `docs/` (or a `design/` folder) before merge. |
| _Optional:_ **listener offload decorator** | The listener non-blocking contract is documented (§3.1.2); a convenience `ChatModelListener.offloaded(...)` decorator for the rare genuinely-blocking listener is a possible future addition — not required. |
