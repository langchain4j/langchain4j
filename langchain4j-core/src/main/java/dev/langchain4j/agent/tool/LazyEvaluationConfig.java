...
public final class LazyEvaluationConfig {
    ...
    private void validateConfiguration() {
        ...
        if (!intersection.isEmpty()) {
            throw new IllegalArgumentException("Tools cannot be configured as both lazy and eager: " + intersection);
        }

        // Validate mode-specific constraints
        if (mode == LazyEvaluationMode.IMMEDIATE_ONLY && !lazyTools.isEmpty()) {
            throw new IllegalArgumentException("IMMEDIATE_ONLY mode cannot have explicit lazy tools configured");
        }
    }
    ...
    public String toString() {
        return "LazyEvaluationConfig{" + "mode="
                + mode + ", lazyTools="
                + lazyTools + ", eagerTools="
                + eagerTools + ", performanceMonitoringEnabled="
