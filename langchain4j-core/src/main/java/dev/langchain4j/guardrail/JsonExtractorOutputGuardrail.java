    ...
    protected Optional<JsonParsingUtils.ParsedJson<T>> deserialize(String llmResponse) {
        try {
            return Optional.of( this.outputClass != null ?
                    extractAndParseJson(llmResponse, this.outputClass.getName(), text -> this.objectMapper.readValue(text, this.outputClass)) :
                    extractAndParseJson(llmResponse, this.outputType.toString(), text -> this.objectMapper.readValue(text, this.outputType)) );
        } catch (Exception e) {
