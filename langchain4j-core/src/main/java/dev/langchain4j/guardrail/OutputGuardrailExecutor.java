...
public class OutputGuardrailExecutor
    ...
    public OutputGuardrailExecutor(OutputGuardrailsConfig config) {
        super(config);
    }
     ...
     * @param guardrails The {@link OutputGuardrail}s to evaluate against
     * @return The {@link OutputGuardrailResult} of the validation
     */
    @Override
    public OutputGuardrailResult execute(OutputGuardrailParams params, Iterable<OutputGuardrail> guardrails) {
        OutputGuardrailResult result = null;
        var accumulatedParams = params;
        var attempt = 0;
        var maxAttempts = (config().maxRetries() > 0) ? config().maxRetries() : 1;

        while (attempt < maxAttempts) {
            result = executeGuardrails(accumulatedParams, guardrails);
