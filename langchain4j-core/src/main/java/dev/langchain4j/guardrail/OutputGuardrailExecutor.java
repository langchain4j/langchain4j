...
public class OutputGuardrailExecutor
    ...
    public OutputGuardrailExecutor(OutputGuardrailsConfig config, OutputGuardrail... guardrails) {
        super(config, guardrails);
    }
    ...
    public OutputGuardrailResult execute(OutputGuardrailParams params) {
        ...
        var attempt = 0;
        var maxAttempts = (config().maxRetries() > 0) ? config().maxRetries() : OutputGuardrailsConfig.MAX_RETRIES_DEFAULT;
