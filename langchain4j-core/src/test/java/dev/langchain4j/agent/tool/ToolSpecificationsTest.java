...
class ToolSpecificationsTest implements WithAssertions {
    ...
    // --- Undetermined parameter names (arg0, arg1, ...) ---

    @Test
    void should_fail_on_autogenerated_parameter_name_when_strict() throws NoSuchMethodException {
        class Tools {
            @Tool
            public void tool(String foo) {}
        }
        Method method = Tools.class.getMethod("tool", String.class);

        assertThatExceptionOfType(IllegalArgumentException.class)
                .isThrownBy(() -> ToolSpecifications.toolSpecificationFrom(method, true))
                .withMessageContaining("-parameters")
                .withMessageContaining("@P(name = \"...\")")
                .withMessageContaining("strictToolParameters(false)");
    }

    @Test
    void should_not_fail_on_parameter_with_explicit_P_name_when_strict() throws NoSuchMethodException {
        class Tools {
            @Tool
            public void tool(@P(name = "foo") String foo) {}
        }
        Method method = Tools.class.getMethod("tool", String.class);

        ToolSpecification ts = ToolSpecifications.toolSpecificationFrom(method, true);

        assertThat(ts.parameters().properties()).containsKey("foo");
    }

    @Test
    void should_not_fail_on_autogenerated_parameter_name_by_default() throws NoSuchMethodException {
        class Tools {
            @Tool
            public void tool(String foo) {}
        }
        ...
        assertThat(ts.parameters().properties()).containsKey("arg0");
    }
     ...
     */
    @SuppressWarnings("unused")
    public static class ChildOverridingToolWithoutAnnotation extends ParentWithOverridableTool {
        @Override
        public int compute(int a) {
            return a * 4;
        }
    ...
    @Test
    void should_discover_tool_when_overriding_method_does_not_repeat_annotation() {
        List<ToolSpecification> specs =
                ToolSpecifications.toolSpecificationsFrom(new ChildOverridingToolWithoutAnnotation());

        assertThat(specs).hasSize(1);
        assertThat(specs.get(0).name()).isEqualTo("compute");
        assertThat(specs.get(0).description()).isEqualTo("parent description");
    }
    ...
    @Test
    void should_discover_abstract_interface_method_implemented_without_tool_annotation() {
        List<ToolSpecification> specs = ToolSpecifications.toolSpecificationsFrom(new ImplementsAbstractToolMethod());

        assertThat(specs).hasSize(1);
        assertThat(specs.get(0).name()).isEqualTo("abstractMethod");
        assertThat(specs.get(0).description()).isEqualTo("abstract tool");
    }

    @SuppressWarnings("unused")
    public static class ImplementsAbstractToolMethodWithOwnAnnotation implements ToolInterfaceWithAbstractMethod {
        @Override
    ...
    @Test
    void should_use_tool_annotation_of_implementation_instead_of_interface_one() {
        List<ToolSpecification> specs =
                ToolSpecifications.toolSpecificationsFrom(new ImplementsAbstractToolMethodWithOwnAnnotation());

        assertThat(specs).hasSize(1);
        assertThat(specs.get(0).name()).isEqualTo("abstractMethod");
        assertThat(specs.get(0).description()).isEqualTo("implementation tool");
    }
