...
class InputGuardrailExecutorTests {
    InputGuardrailExecutor executor = new InputGuardrailExecutor(
    ...
    void allSuccessfulGuardrails(
        ...
        var params = InputGuardrailParams.from(UserMessage.from("test"));
        var result = this.executor.execute(params, spiedGuardrails);
    ...
    void noGuardrails() {
        var params = InputGuardrailParams.from(UserMessage.from("test"));
        var result = this.executor.execute(params);
    ...
    void failedFatal(
        ...
        assertThatExceptionOfType(GuardrailException.class)
                .isThrownBy(() -> this.executor.execute(params, spiedGuardrails))
                .withMessage("The guardrail");

        //        if (howManyFailures == 1) {
        //            assertThat(result).hasSingleFailureWithMessage("failure 1");
        //        } else {
        //            var failureMessages = IntStream.rangeClosed(1, howManyFailures)
        //                    .mapToObj(i -> "failure " + i)
        //                    .toList();
        //
        //            assertThat(result)
        //                    .hasFailures()
        //                    .withFailures()
        //                    .hasSize(howManyFailures)
        //                    .extracting(InputGuardrailResult.Failure::message)
        //                    .containsExactlyElementsOf(failureMessages);
        //        }
    ...
    private static class FatalInputGuardrail extends FailureInputGuardrail<FatalInputGuardrail> {
        ...
        private FatalInputGuardrail(String failureMessage) {
