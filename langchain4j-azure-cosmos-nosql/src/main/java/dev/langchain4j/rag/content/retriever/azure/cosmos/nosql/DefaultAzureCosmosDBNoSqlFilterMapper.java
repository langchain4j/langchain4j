...
public class DefaultAzureCosmosDBNoSqlFilterMapper implements AzureCosmosDBNoSqlFilterMapper {
    ...
    private String mapLogicalOperator(Filter operator) {
        if (operator instanceof And) {
            return format(getLogicalFormat(operator),
                    map(((And) operator).left()),
                    map(((And) operator).right()));
        }
        if (operator instanceof Or) {
            return format(getLogicalFormat(operator),
                    map(((Or) operator).left()),
                    map(((Or) operator).right()));
        }
        if (operator instanceof Not) {
            return format(getLogicalFormat(operator),
                    map(((Not) operator).expression()));
        }
    ...
    private String mapComparisonFilter(Filter filter) {
        ...
        if (filter instanceof FullTextContains)
            return mapFullTextContains((FullTextContains) filter);
        if (filter instanceof FullTextContainsAll)
            return mapFullTextContainsAll((FullTextContainsAll) filter);
        if (filter instanceof FullTextContainsAny)
            return mapFullTextContainsAny((FullTextContainsAny) filter);
    ...
    private String mapIsEqualTo(IsEqualTo filter) {
        return format(getComparisonFormat(filter),
                filter.key(),
                formatValue(filter.comparisonValue()));
    }
    ...
    private String mapFullTextContains(FullTextContains filter) {
        return format("FullTextContains(c.%s, %s)",
                filter.key(),
                formatValue(filter.searchTerm()));
    }

    private String mapFullTextContainsAll(FullTextContainsAll filter) {
        String terms = filter.searchTerms().stream()
                .map(this::formatValue)
                .collect(Collectors.joining(", "));
        return format("FullTextContainsAll(c.%s, %s)",
                filter.key(),
                terms);
    }
    ...
    private String mapFullTextContainsAny(FullTextContainsAny filter) {
        String terms = filter.searchTerms().stream()
                .map(this::formatValue)
                .collect(Collectors.joining(", "));
        return format("FullTextContainsAny(c.%s, %s)",
                filter.key(),
                terms);
    }
