# Open decisions and System One adapters
...
concrete adapter without becoming part of `DecisionModel`. This work
...
The existing experimental artifact and `dev.langchain4j.model.decision`
package remain the location of the provider-neutral interface. The model keeps
`decide(DecisionRequest)`, `decideAsync` with its unsupported default,
...
`DecisionAnswer` becomes a plain interface with `value()`,
...
and covariant scalar `value()`. `DecisionResponse.answers()` remains a
...
`DecisionReceipt` is an explicit client-side snapshot made from a
...
The new public `SystemOneDecisionModel` in a dedicated community
...
`TypeSafeDecisionModel` remains in its existing artifact and retains
...
`ClmDecisionModel` implements the common decision interface, accepts
...
endpoint. The ranking result is not a `DecisionResponse`; ranking
...
parameter type implements `DecisionRequestParameters` and keeps its
...
incompatible with the draft `DecisionAnswer` class; TypeSafe and
