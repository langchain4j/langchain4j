...
In this case every subagent is also provided with an explicit name, so that it can be referenced in the activation condition method, which are annotated with `@ActivationCondition` and specify the name of the subagent they refer to.
