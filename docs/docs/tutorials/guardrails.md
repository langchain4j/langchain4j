...
| [`LlmBasedPromptInjectionGuardrail`](https://github.com/langchain4j/langchain4j/blob/main/langchain4j-guardrails/src/main/java/dev/langchain4j/guardrails/LlmBasedPromptInjectionGuardrail.java) | An input guardrail that detects prompt injection attempts by asking a [`ChatModel`](https://github.com/langchain4j/langchain4j/blob/main/langchain4j-core/src/main/java/dev/langchain4j/model/chat/ChatModel.java) to classify the user input against [OWASP LLM01](https://genai.owasp.org/llmrisk/llm01-prompt-injection/).<br/> - Reasons over intent rather than literal text, so it can flag semantically obfuscated attacks (encoded payloads, ASCII art, leetspeak, paraphrasing) that pattern matching misses.<br/> - Complements `PatternBasedPromptInjectionGuardrail`: chain the cheap pattern gate first, then this classifier.<br/> - Configurable probability threshold, prompt template, and fail-open/fail-closed policy; subclasses can customise the failure message. |
