...
Guardrails that _rewrite_ the text they validate can be declared anywhere in the chain. They do not need to be declared last in order for their rewrite to reach the caller. See [Rewriting guardrails in a chain](#rewriting-guardrails-in-a-chain) for the details.
...
### Rewriting guardrails in a chain

A guardrail can do more than accept or reject the text it receives. By returning **_success with rewrite_** (`successWith(...)`), it replaces the text that the remaining guardrails in the chain, and ultimately the caller, will see. This is how redaction, masking, and format-fixing guardrails work.

Rewrites are composed along the chain according to the following rules. They apply to both input and output guardrails, whether the operation is synchronous or asynchronous/streaming:

- Each guardrail that follows a rewriting guardrail validates the _rewritten_ text, not the original text.
- A rewrite survives a later plain `success()`. A `success()` means _"the text I received is acceptable"_, and the text a later guardrail receives is already the rewritten text, so approving it does not revert to the original text.
- A later rewrite overrides an earlier rewrite. Rewrites are composed in chain order, so the last guardrail to rewrite wins.
- As a result, a rewriting guardrail does not need to be the last guardrail in the chain for its rewrite to be returned to the caller.

:::note
Once any guardrail in an output guardrail chain has rewritten the output, a later **_fatal with retry_** or **_fatal with reprompt_** outcome is not allowed, because the LLM would be asked to produce a new response for text that has already been rewritten. In that situation execution is halted with an `OutputGuardrailException` whose message contains `Retry or reprompt is not allowed after a rewritten output`.
:::
...
|:------------------------------------|:--------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **_success_**                       | `success()`                                       | - The input is valid.<br/> - The next guardrail in the chain is executed.<br/> - The LLM is called if the last guardrail passes.<br/> - If an earlier guardrail in the chain rewrote the user message, **_success_** means that the _rewritten_ user message is valid. The rewrite is preserved. See [Rewriting guardrails in a chain](#rewriting-guardrails-in-a-chain).                                           |
| **_success with alternate result_** | `successWith(String)`                             | - Similar to **_success_** except the user message is altered before proceeding to the next step (next guardrail in the chain or calling the LLM).<br/> - A rewrite made by a guardrail overrides any rewrite made by an earlier guardrail in the chain.                           |
...
Regardless of how they are declared, input guardrails are always executed in the order they appear in the list. A user message rewritten by any guardrail in the list is visible to the guardrails that follow it and, unless a later guardrail rewrites it again, to the LLM.
...
|:---------------------------|:--------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **_success_**              | `success()`                                                         | - The output is valid.<br/> - The next guardrail in the chain is executed. If the last guardrail passes the output is returned to the caller.<br/> - If an earlier guardrail in the chain rewrote the output, **_success_** means that the _rewritten_ output is valid. The rewrite, along with any result object attached to it, is preserved. See [Rewriting guardrails in a chain](#rewriting-guardrails-in-a-chain). |
| **_success with rewrite_** | `successWith(String)` or `successWith(String, Object)`              | -Similar to **_success_** except the output isn't valid in its original form and has been rewritten to make it valid.<br/> - The next guardrail is executed against the rewritten output. If the last guardrail passes the output is returned to the caller.<br/> - A rewrite made by a guardrail overrides any rewrite made by an earlier guardrail in the chain.<br/> - With `successWith(String, Object)`, the result object travels with the rewritten text it was derived from. A later guardrail that rewrites the text only (`successWith(String)`) therefore supersedes that result object and discards it, since the result object no longer corresponds to the current text. A later plain `success()` does not discard it. |
...
Regardless of how they are declared, output guardrails are always executed in the order they appear in the list. An output rewritten by any guardrail in the list is visible to the guardrails that follow it and, unless a later guardrail rewrites it again, to the caller.
