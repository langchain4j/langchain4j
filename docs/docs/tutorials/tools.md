...
ToolExecutor toolExecutor = (toolExecutionRequest, context) -> {
    ...
    return ToolExecutionResult.from(booking.toString());
};
```

The `InvocationContext` provides the chat memory ID and any invocation parameters. For results that contain more than
plain text, use `ToolExecutionResult.builder()` to provide structured content, attributes, or the original result object.
