...
`langchain4j-weaviate` `1.20.1-beta30` and earlier read only the single unnamed vector.
...
This is fixed in `1.20.1-beta30`, which reads both layouts: embeddings that were stored
by an earlier version are returned as well, and no change is needed.

With `1.20.1-beta30` and earlier, create the collection yourself, before using
