...
If your model doesn't support Structured Outputs (JSON schema), you can still use JSON mode to ensure the model's output is valid JSON. This works if your model supports RESPONSE_FORMAT_JSON, such as older versions of OpenAI models or other models compatible with the OpenAI protocol, including gpt-3.5-turbo, gpt-4-*, gpt-4o-* models, deepseek-v3, etc.
